...
2. Female`
      ...
      if (stage === 'create-meal-plan/gender') {
        ...
        const message = `Perfect!, What's your height (FT)?`
        return this.helper.sendTextAndSetCache({
          message, phoneNumber, stage: 'create-meal-plan/height', data: { ...state.data, gender }
        })
      }
      if (stage === 'create-meal-plan/height') {
        const message = `Excellent!, could you share your weight with me (KG)?`
        return this.helper.sendTextAndSetCache({
...
2. Mild activity (intense exercise for at least 20 minutes 1-3 times per week. This may include things like brisk walking, bicycling, jogging, basketball, swimming etc. If you do not exercise regularly, but you maintain a busy 
   life that requires you to walk frequently for long periods, you meet the requirements for this level).
...
5. Extreme activity (Exceedingly active and/ or very demanding activities, such as athlete with an almost unstoppable training schedule, very demanding jobs such as shovelling coal or working long hours on an assembly line).`
      ...
      if (stage === 'create-meal-plan/activity-level') {
        ...
        const message = `Are you managing any of these health conditions?\n
1. None 
2. Hypertension
3. Diabetes
4. Pre-diabetes
5. High Cholesterol`
        return this.helper.sendTextAndSetCache({
          message, phoneNumber, stage: 'create-meal-plan/health-condition', data: { ...state.data, activityLevel: input }
        })
      }

      if (stage === 'create-meal-plan/health-condition') {
        const message = `send result back`
        const healthCondition = input == 1 ? 'none' : input == 2 ? 'hypertension' : input == 3 ? 'diabetes' : input == 4 ? 'diabetes' : input == 5 ? 'highCholesterol' : ''
        if (!healthCondition) {
          sendWhatsAppText({ message: `Please select if you have any health conditions`, phoneNumber });
          return {
            status: 'success'
          }
        }
        return this.helper.sendTextAndSetCache({
          message, phoneNumber, stage: 'create-meal-plan/calculate', data: { ...state.data, healthCondition }
        })
      }

      if (stage === 'create-meal-plan/calculate') {
        const {data:  {}} = state
      }
